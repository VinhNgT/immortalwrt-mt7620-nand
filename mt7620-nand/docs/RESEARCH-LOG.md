...
- **09-06, history rewritten** — the branch was rebuilt from
  `v25.12.1` as ten commits, one per logical change (driver,
  subtarget, device, ECC fix, ImageBuilder feeds, then the project
  folder by goal), replacing the day's incremental series (subtarget
  added then renamed, the `nand.sh` extension added then removed, doc
  fix-ups). Same tree, verified by an empty diff. The previous branch
  is kept on origin as `backup/25.12-pre-rewrite-20260906`; release
  tag `v25.12.1-r1` still points into it.
- **09-06, subtarget parity** — audit of everything that differs
  between `mt7620` and `mt7620_nand` beyond NAND: target.mk and the
  kernel config differ by exactly the fragment; mt7620's six extra
  base-files are per-board case lists the R3 is not in; but three
  subtarget-name gates were missing `mt7620_nand` (Corrections,
  above). Folded into the rebuilt series, same force-push procedure.
- **09-06, sync-script review** — honest review of the two
  sync-keeping scripts. `check-xwrt-drift.sh` printed "No drift" when
  a file could not be fetched at all (a rename at x-wrt would have
  passed silently) and guessed x-wrt's patches directory with a
  hard-coded version; both fixed, and a `--hunks` mode prints x-wrt's
  R3 lines from the four shared files for comparison by eye. The gate
  check's feeds clause never ran in CI because the check preceded the
  feeds install; the workflow now installs feeds first. What stays
  manual, on purpose: mt7620's target.mk, mt7620's base-files gaining
  generic behaviour, and x-wrt's R3 hunks — DEVELOPMENT.md lists them
  as checklist steps in the upstream and x-wrt procedures rather than
  pretending a script could judge them.
- **09-06, note** — the day's history rewrites were a one-off cleanup,
  authorised for that day only. From here on changes are ordinary
  commits on top of the series; DEVELOPMENT.md says so, and its
  fold/force-push recipe is for the occasions when a rewrite is
  decided, not a standing procedure.
...
- **A new subtarget is not the old one plus a feature until every
  gate on the subtarget name says so.** Three places key on
  `TARGET_ramips_mt7620`: `kmod-rt2800-soc`'s dependency (the SoC's
  2.4 GHz driver, a default package), the `mmc-mtk`/`sdhci-mt7620`
  kmods in ramips' `modules.mk`, and the lzma loader's SoC selection.
  The subtarget commit initially listed none of them; the first would
  have shipped R3 images with no 2.4 GHz radio, silently, because
  defconfig drops an unsatisfiable default package without a word.
  Found 09-06 by diffing everything keyed on `mt7620` against
  `mt7620_nand`; fixed in the subtarget commit, the config sync script
  now lists any gate that lacks the name, and the CI and Docker builds
  fail if either radio kmod is missing from the expanded `.config`.
  The package feeds (packages, luci, routing, telephony at
  25.12) gate nothing on the subtarget.
