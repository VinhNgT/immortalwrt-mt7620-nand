...
1–7 and 9–11 derive from x-wrt commits `387988e8c956` (driver) and
`f4fc1766f08a` + follow-ups (device), author Chen Minqiang; 8 is the
subtarget's parity with mt7620, and 12 and 13 are this project's own
fixes. The commits and the short names the
docs use for them:

| short name | commit subject | touch points |
|---|---|---|
| driver | `ramips: add mt7620-nand driver for NAND flash` | 1–3, plus the two "not set" lines |
| subtarget | `ramips: add mt7620_nand subtarget` | 6–8, the subtarget list, the empty image recipe file |
| device | `ramips: add support for Xiaomi Mi Router R3` | 4, 5, 9–11 |
...
The subtarget is the shape the upstream reviewer asked for (below):
everything device-specific lives in
...
one line disabling the new Kconfig symbol.
...
8. `package/kernel/mac80211/ralink.mk`, `modules.mk`,
   `image/lzma-loader/src/Makefile` — the three places that gate on
   the subtarget name list `mt7620_nand` next to `mt7620`:
   `kmod-rt2800-soc` (the SoC's 2.4 GHz driver and a default package;
   without the entry defconfig drops it from the image without a
   word), the `mmc-mtk`/`sdhci-mt7620` kmods, and the lzma loader's
   SoC selection. With these the subtarget is mt7620 plus NAND and
   nothing less, and the config sync script (touch point 7) fails if
   a gate ever lacks the name. mt7620's other base-files need no
   copy: everything in them beyond the sysupgrade scaffolding is a
   per-board case list
9. `mt7620_nand/base-files/lib/upgrade/platform.sh` — nand sysupgrade
   ...
   boot never goes stale (the pattern upstream uses for the ipq806x
   Xiaomi routers). x-wrt does that extra write
   inside its patched `nand.sh` instead; this tree leaves upstream's
   `nand.sh` untouched
10. `mt7620_nand/base-files/etc/board.d/02_network` — switch ports
...
11. `package/boot/uboot-tools/uboot-envtools/files/ramips` —
...
3. **The `mt7620_nand` subtarget.** x-wrt enables `nand` and the UBI
   ...
   mt7620 enables the driver), the one-word subtarget list edit, and
   the `mt7620_nand` entries in the three subtarget-name gates (touch
   point 8), which x-wrt never needed because its R3 stays in mt7620.
...
5. **The second kernel slot is written from `platform.sh`, not
   ...
   This tree leaves `nand.sh` untouched: the R3's `platform.sh` runs
   ...
   Xiaomi routers. Reasons: it keeps the port inside its own
   subtarget, and `nand.sh` is shared by every NAND device and moves
   upstream between point releases (its tar-upgrade function changed
   within the 25.12 line), so a hunk there would be the port's main
   rebase risk. How the decision was reached is in
...
the partition is rewritten. Goes upstream as part of the driver
(DEVELOPMENT.md, *Submitting upstream*); x-wrt, which maintains the
driver, would be told too. The
...
already ships in `keys/`. Not part of the planned upstream
submission (DEVELOPMENT.md): it is an ImmortalWrt build-system matter
of its own.
...
framework. The subtarget exists here (the subtarget commit above), so
the remaining gap to a mainline-quality submission is the driver
rewrite. Encouragingly, mt7621 already ships
`FEATURES+=nand` with 46 UBI recipes, so the
ubinize/UBI/nand_do_upgrade pipeline is proven in this target tree.
What would be submitted is the NAND support only — the driver with
the ECC fix folded in, the subtarget and the R3 — re-cut as three
commits the way [DEVELOPMENT.md](DEVELOPMENT.md#submitting-upstream)
describes. The IB-feeds commit stays out of it.
